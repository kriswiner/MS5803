 ...
 *  
 ...
 https://www.te.com/commerce/DocumentDelivery/DDEController?Action=srchrtrv&DocNm=MS5803-01BA&DocType=Data+Sheet&DocLang=English
...
uint16_t Pcal[8];         // calibration constants from MS5803 PROM registers
unsigned char nCRC;       // calculated check sum to ensure PROM integrity
uint32_t D1 = 0, D2 = 0;  // raw MS5803 pressure and temperature data
double dT, OFFSET, SENS, TT2, OFFSET2, SENS2;  // First order and second order corrections for raw MS5803 temperature and pressure data
    
double Temperature, Pressure; // stores MS5803 pressures sensor pressure and temperature
...
void setup()
{
  ...
  i2c_0.I2Cscan();                                      // should detect MS5803 at 0x76
  delay(1000);
  ...
  unsigned char refCRC = Pcal[7] & 0x000F;
  
  nCRC = MS5803.checkCRC(Pcal);  //calculate checksum to ensure integrity of MS5803 calibration data
...
// Second order corrections
    if(Temperature > 20) 
    {
      TT2 = 0; // correction for high temperatures
      OFFSET2 = 0;
      SENS2 = 0;
    }
    ...
    {
      TT2     = dT*dT/pow(2, 31); 
      OFFSET2 = 3*(100*Temperature - 2000)*(100*Temperature - 2000);
      SENS2   = 7*(100*Temperature - 2000)*(100*Temperature - 2000)/8;
    } 
    if(Temperature < -15)                      // correction for very low temperature
    {
      OFFSET2 = OFFSET2 + 7*(100*Temperature + 1500)*(100*Temperature + 1500);
      SENS2 = SENS2 + 2*(100*Temperature + 1500)*(100*Temperature + 1500);
    }
